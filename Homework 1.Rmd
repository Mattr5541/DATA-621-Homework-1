...
money_train_comp <- money_train %>% dplyr::select(INDEX, TEAM_BATTING_SO, TEAM_BATTING_HBP)
money_train_mod_comp <- money_train_mod %>% dplyr::select(INDEX, TEAM_BATTING_SO, TEAM_BATTING_HBP)
...
# Apply log transformation safely by adding 1 to avoid log(0)
...
```{r}
model3 = lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_DP + log_TEAM_PITCHING_SO + log_TEAM_FIELDING_E + TEAM_BATTING_H:TEAM_BASERUN_SB + TEAM_PITCHING_SO:TEAM_FIELDING_DP, data = money_train_mod)
...
Note: I do acknowledge the warning received when running both the AIC and BIC
comparisons, so here is a quick check to show the number of observations associated
with each model.

```{r}
# Check the number of observations for each model
n_obs_model1 <- nobs(model1)
n_obs_model2 <- nobs(model2)
n_obs_model3 <- nobs(model3)

# Print the number of observations for each model
print(n_obs_model1)
print(n_obs_model2)
print(n_obs_model3)

print((n_obs_model1-n_obs_model3)/n_obs_model1)
```
